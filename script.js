    ...
    Array.from(e.target.files).forEach(file=>{
        ...
        reader.onload=function(ev){
            ...
            div.querySelector(".generate3DBtn").addEventListener("click", ()=>{
                generate3DPreview(ev.target.result);
            });
...
// 3D Preview
function generate3DPreview(imageSrc){
...
function generatePDF(invoiceOverride){
    const { jsPDF } = window.jspdf;
    const doc = new jsPDF('p','pt','a4');
    const clientName = invoiceOverride?.clientName || document.getElementById("clientName").value;
    const invoiceNumber = invoiceOverride?.invoiceNumber || document.getElementById("invoiceNumber").value;
    const invoiceDate = invoiceOverride?.invoiceDate || document.getElementById("invoiceDate").value;
    const clientGST = invoiceOverride?.clientGST || document.getElementById("clientGST").value;
    const companyGST = invoiceOverride?.companyGST || document.getElementById("companyGST").value;
    ...
    // Header
    doc.setFillColor(46,125,50); doc.rect(0,0,595,50,'F');
    ...
    // Footer
    doc.setFillColor(46,125,50); doc.rect(0,770,595,50,'F');
    ...
    // Watermark
    doc.setTextColor(200,200,200); doc.setFontSize(60);
    doc.text("Varshith Interior Solutions",150,400,{angle:45,opacity:0.1});

    doc.save(`Invoice_${invoiceNumber}.pdf`);

    invoiceData[invoiceNumber] = {
        clientName,invoiceNumber,invoiceDate,clientGST,companyGST,
        items: rows,
        ...
        designs: Array.from(designList.querySelectorAll(".design-item")).map(d=>({src:d.querySelector(".design-thumb").src,name:d.querySelector(".design-name").value}))
    };
    localStorage.setItem("invoiceData",JSON.stringify(invoiceData));
}

// Recover Invoice
function displayRecovery(results){
    recoveryResults.innerHTML='';
    if(results.length===0){ recoveryResults.textContent='No invoice found'; return; }
    results.forEach(inv=>{
        const div=document.createElement('div');
        div.innerHTML=`Invoice No: ${inv.invoiceNumber}, Client: ${inv.clientName}, Date: ${inv.invoiceDate} <button>Download PDF</button>`;
        div.querySelector('button').addEventListener('click',()=>generatePDF(inv));
        recoveryResults.appendChild(div);
    });
}
...
document.getElementById("searchByInvoiceNumber").addEventListener("click",()=>{
    ...
    const res=num&&invoiceData[num]?[invoiceData[num]]:[]; displayRecovery(res);
});
document.getElementById("searchByClientName").addEventListener("click",()=>{
    const name=document.getElementById("searchClientName").value.toLowerCase();
    const res=Object.values(invoiceData).filter(i=>i.clientName.toLowerCase().includes(name));
    displayRecovery(res);
});
document.getElementById("searchByInvoiceDate").addEventListener("click",()=>{
    ...
    const res=Object.values(invoiceData).filter(i=>i.invoiceDate===date);
    displayRecovery(res);
});
