...
// Generate PDF
document.getElementById("generatePDFBtn").addEventListener("click", ()=>saveInvoicePDF());
...
// Recover Invoice Search
function renderRecoveryResults(results){
    recoveryResults.innerHTML = "";
    if(results.length===0){ recoveryResults.textContent="No invoices found"; return; }
    results.forEach(inv=>{
        const div = document.createElement("div");
        div.innerHTML = `<strong>${inv.invoiceNumber}</strong> - ${inv.clientName} - ${inv.invoiceDate} <button>Download PDF</button>`;
        div.querySelector("button").addEventListener("click", ()=>{ populateInvoice(inv); saveInvoicePDF(); });
...
function populateInvoice(inv){
    ...
    invoiceTableBody.innerHTML="";
    inv.items.forEach(r=>{ addRow(r[0],r[1],parseFloat(r[2]),parseFloat(r[3])); });
    designList.innerHTML="";
    inv.designs.forEach(d=>{
        const div = document.createElement("div"); div.className="design-item";
        div.innerHTML = `<img src="${d.src}" class="design-thumb">
                         <div class="design-info"><input class="design-name" value="${d.name}">
                         <div class="design-controls"><button class="generate3DBtn">Generate 3D</button>
                         <button class="removeDesignBtn">Remove</button></div></div>`;
        div.querySelector(".removeDesignBtn").addEventListener("click", ()=>div.remove());
        designList.appendChild(div);
    });
    recalcInvoice();
...
document.getElementById("searchByInvoiceNumber").addEventListener("click", ()=>{
    const val = document.getElementById("searchInvoiceNumber").value;
    const res = Object.values(invoiceData).filter(i=>i.invoiceNumber===val);
    renderRecoveryResults(res);
});
document.getElementById("searchByClientName").addEventListener("click", ()=>{
    const val = document.getElementById("searchClientName").value.toLowerCase();
    const res = Object.values(invoiceData).filter(i=>i.clientName.toLowerCase().includes(val));
    renderRecoveryResults(res);
});
document.getElementById("searchByInvoiceDate").addEventListener("click", ()=>{
    const val = document.getElementById("searchInvoiceDate").value;
    const res = Object.values(invoiceData).filter(i=>i.invoiceDate===val);
    renderRecoveryResults(res);
});
