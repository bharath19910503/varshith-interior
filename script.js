...
function addRow(item='', material='', qty=1, price=0){
    ...
    tr.innerHTML = `<td><input value="${item}"></td><td><input value="${material}"></td><td><input type="number" value="${qty}" min="1"></td><td><input type="number" value="${price}" min="0"></td><td class="amount">${(qty*price).toFixed(2)}</td><td><button class="deleteBtn">Delete</button></td>`;
...
document.getElementById("clearRowsBtn").addEventListener("click", ()=>{
    ["clientName","invoiceNumber","invoiceDate","clientGST","companyGST"].forEach(id=>document.getElementById(id).value='');
    invoiceTableBody.innerHTML=''; designList.innerHTML=''; recalcInvoice();
});
...
document.getElementById("upload2D").addEventListener("change", e=>{
    Array.from(e.target.files).forEach(file=>{
        ...
        reader.onload = function(ev){
            ...
            div.innerHTML = `<img src="${ev.target.result}" class="design-thumb"><div class="design-info"><input class="design-name" value="${file.name}"><div class="design-controls"><button class="generate3DBtn">Generate 3D</button><button class="removeDesignBtn">Remove</button></div></div>`;
            div.querySelector(".removeDesignBtn").addEventListener("click", ()=>div.remove());
            div.querySelector(".generate3DBtn").addEventListener("click", ()=>alert("3D preview functionality can be added here."));
...
document.getElementById("generatePDFBtn").addEventListener("click", ()=>{
    ...
    invoiceData[invoiceNumber]={clientName,invoiceNumber,invoiceDate,clientGST,companyGST,items:[],designs:[]};

    doc.setFillColor(46,125,50); doc.rect(0,0,595,50,'F');
    doc.setFontSize(16); doc.setTextColor(255,255,255); doc.text("Varshith Interior Solutions",40,30);
    ...
    invoiceTableBody.querySelectorAll("tr").forEach(tr=>{
        const row = [];
        tr.querySelectorAll("input").forEach((inp,i)=>{if(i===2||i===3) row.push(parseFloat(inp.value).toLocaleString('en-IN',{minimumFractionDigits:2})); else row.push(inp.value);});
        row.push(tr.children[4].textContent);
        rows.push(row);
    });
    invoiceData[invoiceNumber].items = rows;
    ...
    designList.querySelectorAll(".design-item").forEach(div=>{
        ...
        invoiceData[invoiceNumber].designs.push({src:img.src,name});
        doc.text(name,40,yOffset); doc.addImage(img.src,'JPEG',40,yOffset+5,100,75); yOffset+=85;
        if(yOffset>700){doc.addPage();yOffset=40;}
    });

    doc.setFillColor(46,125,50); doc.rect(0,770,595,50,'F'); doc.setTextColor(255,255,255); doc.setFontSize(10);
